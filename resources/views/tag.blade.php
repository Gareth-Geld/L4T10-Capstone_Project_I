...
Displays appropriate posts depending on the tag being passed through from the Route in the URL-->
<html lang="en">
<head>
    <meta charset="UTF-8">
...
@include('cookie-notice')
    ...
    @php
        $count = 0;
    @endphp
    @foreach($tag as $tager)
    @foreach($posts as $post)
    @foreach ($articleTags as $Atag)
    @if($tager->TAG_ID === $Atag -> TAG_ID)
    @if($Atag->ARTICLE_ID === $post->ARTICLE_ID)
        <div class="col">
