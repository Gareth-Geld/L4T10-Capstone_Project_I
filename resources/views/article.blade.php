...
It displays the article by the ID being passed through the URL from the route Only displays one post
Applies the appropriate links associated with each category and tag that take them to their respective blade pages -->
<html lang="en">
<head>
    <meta charset="UTF-8">
...
@if($post)
    <div class="postBlock">
        <p class="headingPost">{{ $post->TITLE }}</p>
        <p class="contentPost">{{ $post->CONTENT }}</p>
        <div class="spacer"></div>
        <p class="postDetails">{{ $post->DATE_CREATED }} ||
